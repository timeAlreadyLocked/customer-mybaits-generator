...
//        commentGenerator.addComment(selectOneElement);
        ...
        if (this.getContext().getJdbcConnectionConfiguration().getConnectionURL().contains(":sqlserver:"))
            selectOneElement.addElement(new TextElement("top 1"));
        ...
        if (this.getContext().getJdbcConnectionConfiguration().getConnectionURL().contains(":mysql:"))
            // 只查询一条
...
//        commentGenerator.addComment(selectOneWithBLOBsElement);
        ...
        if (this.getContext().getJdbcConnectionConfiguration().getConnectionURL().contains(":sqlserver:"))
            selectOneWithBLOBsElement.addElement(new TextElement("top 1"));
        StringBuilder sb = new StringBuilder();
        if (stringHasValue(introspectedTable.getSelectByExampleQueryId())) {
            sb.append('\'');
            sb.append(introspectedTable.getSelectByExampleQueryId());
